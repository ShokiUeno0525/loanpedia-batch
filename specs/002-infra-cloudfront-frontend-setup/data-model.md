...
3. **OAC**: CloudFrontからS3へのアクセス制御

### Phase 2: セキュリティリソース

4. **WAF WebACL**: Webアプリケーションファイアウォール

### Phase 3: 配信リソース

5. **CloudFrontDistribution**: CDNディストリビューション（OAC、WAF、LogBucketを参照）
6. **S3バケットポリシー**: FrontendBucketへのOACアクセス許可（CloudFrontディストリビューション作成後）

### Phase 4: DNS設定

7. **Route53 ARecord**: ドメイン名をCloudFrontにマッピング
...
本スタックは以下のOutputsを提供します（他のスタックやCI/CDから参照可能）：
