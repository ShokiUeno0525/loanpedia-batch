...
# 可能な限りパッケージインポートに統一（/var/task 配下をモジュールルートとして扱う）
# Lambda では /var/task が sys.path に含まれるため、以下のインポートで解決可能。
try:
    ...
    # ローカル直叩きなどでパスが崩れている場合に備え、最小限のフォールバック
    handler_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(os.path.dirname(handler_dir))  # loanpedia_scraper 直下
    for p in [project_root, os.path.join(project_root, "scrapers"), os.path.join(project_root, "database")]:
