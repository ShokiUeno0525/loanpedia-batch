...
class AomorimichinokuEducationDeedScraper:
    ...
    def _extract_interest_rates(self, soup, item):
        ...
        # 証書貸付型特有の金利パターンを検索
        rate_patterns = [
            (r"店頭表示金利.*?年\s*(\d+\.\d+)\s*[%％].*?引下後.*?年\s*(\d+\.\d+)\s*[〜～]\s*(\d+\.\d+)\s*[%％]", "店頭表示金利からの優遇後"),
            (r"固定金利.*?年\s*(\d+\.\d+)\s*[%％].*?変動金利.*?年\s*(\d+\.\d+)\s*[〜～]\s*(\d+\.\d+)\s*[%％]", "固定・変動金利範囲"),
            (r"年\s*(\d+\.\d+)\s*[%％]\s*[〜～から]\s*[引下後]*\s*年\s*(\d+\.\d+)\s*[〜～]\s*(\d+\.\d+)\s*[%％]", "優遇後金利範囲"),
            (r"変動金利.*?年\s*(\d+\.\d+)\s*[%％].*?年\s*(\d+\.\d+)\s*[〜～]\s*(\d+\.\d+)\s*[%％]", "変動金利範囲"),
        ...
        for table in tables:
            ...
            for row in rows:
                ...
                for i, cell in enumerate(cells):
                    ...
                    if any(keyword in cell_text for keyword in ['金利', '利率', '年利']):
                        if i + 1 < len(cells):
                            next_cell = cells[i + 1].get_text().strip()
                            rate_match = re.search(r'(\d+\.\d+)[〜～]?(\d+\.\d+)?[%％]', next_cell)
                            if rate_match:
                                if rate_match.group(2):  # 範囲がある場合
                                    item["min_interest_rate"] = float(rate_match.group(1))
                                    item["max_interest_rate"] = float(rate_match.group(2))
                                else:  # 単一の値の場合
                                    item["min_interest_rate"] = float(rate_match.group(1))
                                    item["max_interest_rate"] = float(rate_match.group(1))
                                logger.info(f"✅ テーブルから金利抽出: {item['min_interest_rate']}% - {item['max_interest_rate']}%")
                                return
    ...
    def _extract_loan_periods(self, soup, item):
        ...
        period_patterns = [
            (r"(\d+)\s*ヶ月\s*以上\s*(\d+)\s*年\s*以内", "月年混合"),
        ...
        for pattern, pattern_type in period_patterns:
            ...
            if match:
                ...
                logger.info(f"✅ 融資期間: {item.get('min_loan_term_months', 0)}ヶ月 - {item.get('max_loan_term_months', 0)}ヶ月")
                return
...
def main():
    ...
    if result and result.get("scraping_status") == "success":
        ...
        print(f"融資期間: {result.get('min_loan_term_months')}ヶ月 - {result.get('max_loan_term_months')}ヶ月")
